...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.predict_action = nn.Sequential(
            *([nn.Linear(hidden_size, max_action_num * self.act_dim)] + ([nn.Tanh()] if action_tanh else []))
        )
    ...
    def forward(self, states, actions, rewards=None, targets=None, returns_to_go=None, timesteps=None,
                attention_mask=None, action_num=10, out_action_num=5):
        ...
        if self.sar:
            ...
            multi_action_preds = action_preds.view(B, T, action_num, act_dim)
            state_preds = self.predict_state(x[:, 2])
            rewards_preds = None

        return state_preds, multi_action_preds[:, :, :out_action_num, :], rewards_preds

    def get_action(self, critic, states, actions, rewards=None, returns_to_go=None, timesteps=None, action_num=10,
                   out_action_num=3, pre_critic=None, **kwargs):  # TODO 推理3
        ...
        states_tmp = states_repeated[:, -self.max_length:, ]
        states_padding = torch.cat(
            [torch.zeros((states.shape[0], self.max_length - states_tmp.shape[1], out_action_num, self.state_dim),
                         device=states.device), states_tmp],
            dim=1).to(dtype=torch.float32)

        # TODO 这里为什么要重复50次呢?
        bs = returns_to_go.shape[0]
        returns_to_go = returns_to_go.reshape(bs, -1, 1).repeat_interleave(repeats=50 // bs, dim=0)
        returns_to_go = torch.cat([returns_to_go, torch.randn((50 - returns_to_go.shape[0], returns_to_go.shape[1], 1),
                                                              device=returns_to_go.device)], dim=0)
        # 这里的意思是说如果条数小于50要补充够50, 要是本来就有50调就没有变化. 单位提在于,我不就是重复够50次了吗.. 疑惑..可能是别的环境RTG不能被50整除

        if self.max_length is not None:
            states = states[:, -self.max_length:]
            actions = actions[:, -self.max_length:]
            returns_to_go = returns_to_go[:, -self.max_length:]  # 这里做截断的话两个预定义的RTG都在的, 50组里都是25个, 截断的是注意力机制的序列
            ...
            attention_mask = None
        # 并不是很能理解下面这一行的行为, 为何要从第二行开始呢, 加这个随机噪音又是图什么呢?
        returns_to_go[bs:, -1] = returns_to_go[bs:, -1] + torch.randn_like(returns_to_go[bs:, -1]) * 0.1
        if not self.rtg_no_q:  # 按我的理解的话, 应该是想用Q值来替换RTG, 但是为什么要用-2来替换,不用-1呢, 刚开始的-1:是因为第一个50都是重复的,选哪个都一样
            if pre_critic:
                returns_to_go[-1, -1] = pre_critic(states[-1:, -2], actions[-1:, -2]).flatten() - rewards[-1, -2] / self.scale
            else:
                returns_to_go[-1, -1] = critic.q_min(states[-1:, -2], actions[-1:, -2]).flatten() - rewards[
                    -1, -2] / self.scale
        _, action_preds, return_preds = self.forward(states, actions, rewards, None, returns_to_go=returns_to_go,
                                                     timesteps=timesteps, attention_mask=attention_mask,
                                                     action_num=action_num, out_action_num=out_action_num, **kwargs)

        # 好像懂了, 只需要最后时刻的state和action就够了
        # state_rpt = states[:, -1, :]
        state_rpt = states_padding[:, -1, :]
        action_preds = action_preds[:, -1, :]

        if pre_critic:
            q_value = pre_critic(state_rpt, action_preds).flatten()
        else:
            q_value = critic.q_min(state_rpt, action_preds).flatten()
        idx = torch.multinomial(F.softmax(q_value, dim=-1), 1)

        time_step = int(idx // out_action_num)
        action_index = int(idx % out_action_num)

        if not self.infer_no_q:
            return action_preds[time_step][action_index]
        else:
