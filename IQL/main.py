...
def main(args):
    ...
    def eval_policy():
        ...
        wandb.log({"return mean": eval_returns.mean()})
        wandb.log({'return std': eval_returns.std()})
        wandb.log({'normalized return mean': normalized_returns.mean()})
        wandb.log({'normalized return std': normalized_returns.std()})
        log.row({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log-dir', type=str, default="IQL_runs/")
    ...
    wandb_name = parser.parse_args().env_name + '-' + current_time

    wandb.init(project="IQL", name=wandb_name)
