...
class EnsembleDynamics(BaseDynamics):
    ...
    def step(  # TODO 预测网络
        ...
        obs_act = np.concatenate([obs.cpu().numpy(), action.cpu().numpy()], axis=-1)
        obs_act = self.scaler.transform(obs_act)
        mean, logvar = self.model(obs_act)  # 均值和对数方差
        mean = mean.cpu().numpy()
        logvar = logvar.cpu().numpy()
        mean[..., :-1] += obs.cpu().numpy()  # -1的维度是reward, 这样设计可能是模型预测的是增量, 所以需要把obs加上.
        std = np.sqrt(np.exp(logvar))

        ensemble_samples = (mean + np.random.normal(size=mean.shape) * std).astype(np.float32)  # 根据均值和方差产生一个正态分布


        num_models, batch_size, _, _ = ensemble_samples.shape
        model_idxs = self.model.random_elite_idxs(batch_size)
        samples = ensemble_samples[model_idxs, np.arange(batch_size)]  # 会根据model_idx的序列选择对应的elite模型
        # 把Samples拆分成obs和reward
        next_obs = samples[..., :-1]
        reward = samples[..., -1:]
        # terminal = self.terminal_fn(obs, action, next_obs)  # 判断环境是否终止
        info = {}
        info["raw_reward"] = reward

        if self._penalty_coef:  # 不确定性惩罚
            if self._uncertainty_mode == "aleatoric":
                penalty = np.amax(np.linalg.norm(std, axis=-1), axis=0)  # 使用标准差的最大范数作为惩罚
            elif self._uncertainty_mode == "pairwise-diff":
                next_obses_mean = mean[..., :-1]
                next_obs_mean = np.mean(next_obses_mean, axis=0)
                diff = next_obses_mean - next_obs_mean
                penalty = np.amax(np.linalg.norm(diff, axis=-1), axis=0)  # 使用均值之间的差异作为惩罚
            elif self._uncertainty_mode == "ensemble_std":
                next_obses_mean = mean[..., :-1]
                penalty = np.sqrt(next_obses_mean.var(0).mean(1))  # 使用均值的方差作为惩罚
            else:
                raise ValueError
            ...
            coef_penalty = self._penalty_coef * penalty

        return next_obs, coef_penalty, info
    ...
    def train(
        ...
        # PyTorch 的 random_split 函数随机分割数据集。这个函数接受一个序列（这里是 range(data_size)，代表所有样本的索引）和一个包含每个部分大小的元组（这里是 (train_size, holdout_size)）。函数返回两个对象，分别对应训练集和留出集的索引
        train_splits, holdout_splits = torch.utils.data.random_split(range(data_size), (train_size, holdout_size))
        # 使用 train_splits.indices，这是一个包含训练集样本索引的列表，从 inputs 和 targets 中提取出训练数据。
        ...
        epoch = 0
        cnt = 0
        logger.log("Training dynamics:")  # TODO
        while True:
            epoch += 1
    ...
    def learn(  # TODO dynamics的训练
        ...
        for batch_num in range(int(np.ceil(train_size / batch_size))):
            ...
            # mean这个函数预测的是下一个状态和现在这个状态的差, 以及reward
