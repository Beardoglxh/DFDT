...
from offlinerlmoup.modules import DEnsembleDynamicsModel
...
from offlinerlmoup.utils.logger import Logger as mLogger
from offlinerlmoup.utils.logger import make_log_dirs
from config import predefined_parameters
from diffusion_config import df_predefined_parameters
import wandb
...
def experiment(
    ...
    action_num = variant['action_num']
    max_uncertainty = variant['max_uncertainty']
    ...
    def eval_episodes(target_rew):  # TODO 推理
        def fn(model, critic, action_num, out_action_num, pre_critic):
            returns, lengths = [], []
            for _ in range(num_eval_episodes):
                with torch.no_grad():
                    ret, length = evaluate_episode_rtg(
                        ...
                        out_action_num=out_action_num,
                        pre_critic=pre_critic
                    )
                returns.append(ret)
                lengths.append(length)
            wandb.log({
                'return mean': np.mean(returns),
                'return_std': np.std(returns),
                'length_mean': np.mean(lengths),
                'normalized_score': 100 * env.get_normalized_score(np.mean(returns))
            })
            return {  # eval了10次, 然后取得均值
    ...
    #         device=args.device,
    #         times=args.dynamics_times  # TODO 应该是每次都要改
    #     )
    ...
    #     termination_fn = get_termination_fn(task=gym_name)  # 一个判断环境是否终止的函数, 方便以后调用
    ...
    #         log_dirs = make_log_dirs(gym_name, args.algo_name, args.seed, vars(args),
    ...
    #         }
    #
    #         mlogger = mLogger(log_dirs, output_config)
    #         dynamics.train(real_buffer.sample_all(), mlogger)
    #     return dynamics, real_buffer, fake_buffer

    # dynamics, real_buffer, fake_buffer = train_dynamics()

    diffusion_path = "./diffusion_model/pretrained_model/diffusion_model-{}-{}-v{}.pth".format(env_name, dataset, dversion)
    diffusion_dynamic = DiffusionWrapper(diffusion_path, device=device)
    ...
    if variant["pretrained_critic"]:
        iql_model_path = './IQL/pretrained_iql_model/'
        iql_model_name = 'iql-{}-{}-v{}.pt'.format(env_name, dataset, dversion)
        pretrained_iql_model_path = os.path.join(iql_model_path, iql_model_name)
        iql = torch.load(pretrained_iql_model_path)
        pre_critic = iql.qf
        pre_critic.to(device)
        pre_critic.eval()
    ...
    for iter in range(variant['max_iters']):
        outputs = trainer.train_iteration(num_steps=variant['num_steps_per_iter'], logger=logger,
                                          iter_num=iter + 1, log_writer=writer, action_num=action_num,
                                          max_uncertainty=max_uncertainty, pre_critic=pre_critic)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=200)  # seed
    parser.add_argument('--env', type=str, default='halfcheetah')  # halfcheetah, hopper, walker2d
    ...
    # TODO cmd
    # parser.add_argument("--action_num", default=3, type=int)
    # parser.add_argument("--dynamics_times", default=18, type=int)
    # parser.add_argument("--max_uncertainty", default=1.0, type=int)
    parser.add_argument("--load_dynamics_path",
                        default=False,
                        type=str)
    ...
    parser.add_argument("--penalty-coef", type=float, default=0.25)
    parser.add_argument("--rollout-batch-size", type=int, default=50000)
    parser.add_argument("--rollout-length", type=int, default=5)
    parser.add_argument("--model-retain-epochs", type=int, default=5)
    parser.add_argument("--algo-name", type=str, default="seaDT")
    parser.add_argument('--pretrained_critic', default=True)
    # parser.add_argument("--expectile", type=float, default=0.8)

    args = parser.parse_args()
    # parameters = predefined_parameters.get((args.env, args.dataset))
    # if parameters:
    #     args.eta = parameters["eta"]
    #     args.grad_norm = parameters["grad_norm"]
    #     args.action_num = parameters["action_num"]
    #     args.dynamics_times = parameters["dynamics_times"]
    #     args.max_uncertainty = parameters["max_uncertainty"]
    parameters = df_predefined_parameters.get((args.env, args.dataset))
    if parameters:
        args.eta = parameters["eta"]
        args.grad_norm = parameters["grad_norm"]
        args.action_num = parameters["max_action_num"]
        args.max_uncertainty = parameters["max_uncertainty"]
        args.expectile = parameters["expectile"]


    current_time = datetime.datetime.now().strftime("%Y%m%d-%H:%M:%S")
    wandb_name = args.env + '-' + args.dataset + '-' + current_time + '-' + str(args.action_num) + '-' + str(args.max_uncertainty) + '-' + str(args.expectile)

    wandb.init(mode="disabled")
    # wandb.init(project="Diffusion SeaDT", name=wandb_name)
